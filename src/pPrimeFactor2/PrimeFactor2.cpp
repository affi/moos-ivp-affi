...
bool PrimeFactor2::Iterate()
{
  for (int i = 1; i < buffer_length; i++) {
	...
	",username=" + num_values.at(i).username;
      ...
      EnteredValueClear(num_values.at(i));
    }
    else if ((num_values.at(i).current_num != 0) &&
	     (num_values.at(i).current_num != 1)) {
      PrimeFactorization(num_values.at(i),calculations);
    }
  }
...
void PrimeFactor2::PrimeFactorization(struct entered_value& item, int& calc_num) {
  int i = 2;
...
//---------------------------------------------------------
// Procedure: EnteredValueClear

void PrimeFactor2::EnteredValueClear(struct entered_value& item) {
  item.orig_num           = 0;
  item.current_num        = 0;
