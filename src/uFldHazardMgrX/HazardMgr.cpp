...
bool HazardMgr::OnNewMail(MOOSMSG_LIST &NewMail)
{
    ...
    for(p=NewMail.begin(); p!=NewMail.end(); p++) {
        ...
        else if (key == "HZDREPORT_COMPLEMENT") {
...
void HazardMgr::registerVariables()
{
    ...
    m_Comms.Register("HZDREPORT_COMPLEMENT");
...
bool HazardMgr::handleMailDetectionReport(string str)
{
    ...
    if(ix == -1) {
        ...
        NodeMessage msgToSend;
        
        msgToSend.setSourceNode(m_host_community);
        msgToSend.setDestNode("all");
        //msgToSend.setVarName("UHZ_DETECTION_REPORT");
        msgToSend.setVarName("HZDREPORT_COMPLEMENT");
        msgToSend.setStringVal(str);
        
        string outString = msgToSend.getSpec();
        
        Notify("NODE_MESSAGE_LOCAL", outString);
    }
...
void HazardMgr::handleMailReportRequest()
{
    ...
    //    msgToSend.setDestNode("all");
    //    msgToSend.setVarName("HZDREPORT_COMPLEMENT");
    //    msgToSend.setStringVal(summary_report);
    //
    //    string outString = msgToSend.getSpec();
    //
    //    Notify("NODE_MESSAGE_LOCAL", outString);
